...
**The gates live in rux-ds.** Compose the page in this checkout and run
`npm run verify` in `rux-ds/` before it leaves; that catches the invented class
and the uncompiled component while it is still cheap. For an app's pages, the
repository's `npm run check` runs rux-ds's shared check.
...
3. **The sprite must be inlined.** `<use>` against an external file is blank in
   ...
   **silently**: a fully styled page with no icons. Keep the `SPRITE:BEGIN` and
   ...
   and marked rux-ds root pages, and the repository's `npm run build` refreshes
   app pages.
