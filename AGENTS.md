...
A colour goes in the app's `theme.css` inside a `[data-theme]` block; a
component rule in its `overrides.css` at Carbon's specificity; a
...
| Notes | pages are generated by `notes/tools/build.mjs` from `notes/data/atlas/`, both committed and never hand-edited. Nothing on a page may identify a person, an environment, a client or a vendor document; the fix is upstream in atlas, never a filter here |
| the scheduler | `scheduler/docs/` — inventories and plans; `scheduler-*` is its own prefix |
| the database | no repository, see the opening; `docs/database-access.md` is who may read it and how to check that has not slipped; the identity plan is in the archived `rux-backend` repository; the publishable key in `account.js` and `data.js` is not a secret |
