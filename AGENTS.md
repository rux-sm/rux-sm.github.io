...
| the database | no repository, see the opening; the identity plan is `docs/identity-plan.md` in the archived `rux-backend`; the publishable key in `account.js` and `sch-data.js` is not a secret |
...
| Kind | Where | Holds |
| :--- | :--- | :--- |
| README | `README.md` at the root and in each app folder | what it is, how to run and check it |
| Rules | this `AGENTS.md`, which `CLAUDE.md` imports | the rules, each with one line of reason |
| Status | `docs/status.md` | open work, a line or two per item with its reason |
| Reference | an app's `docs/` | a contract, procedure, spec or inventory something uses today |
| Plan | `docs/` | a decision while it is being made |

- **Present tense.** No dates of passes, and no "was", "until", "corrected"
  or "revised". A date stays only when it is part of a fact, like when a value
  was measured.
- **Done is deleted.** A finished status item, plan step or answered question
  goes in the commit that finishes it. A plan goes when it lands or is dropped.
- **One home per fact.** Other documents link to it and never restate it.
- **A reason is one sentence.** The argument, the measurements and the
  rejected options go in the commit message.
- **A correction replaces the wrong sentence.** It never adds a note beside it.
- **Short.** A README fits on about a screen. A document past about 300 lines
  is split or cut.
- **No other kind.** No logs, audits, memos or decision records. A new kind
  changes this section first.
- **Comments too.** A code comment says what the code does and why, in the
  present tense, and is brought in line when it is touched.
