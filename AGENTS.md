...
Git history is the record. A commit message says what changed and why, and no
file here logs passes, dates or how something came to be. A document is one of
four kinds:

- a `README.md` per app: what it is and how to run it;
- this `AGENTS.md`: the rules;
- `docs/status.md`: the one list of open work, each item with its reason,
  deleted when it is done;
- a reference something here actually uses: a contract, a procedure, an
  inventory or a gate's input.

A plan being decided may sit in `docs/` until it lands, and then it goes. A
document that stops describing the present is deleted, not kept as history.
