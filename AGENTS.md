...
| rux-ds | `rux-ds/README.md` for its build, gates and pages; `rux-ds/docs/` for its references |
| Notes | pages are generated by `rux-ln-notes/tools/build.mjs` from `rux-ln-notes/data/guides/`, both committed and never hand-edited. Nothing on a page may identify a person, an environment, a client or a vendor document; the fix is upstream in atlas, never a filter here |
| the scheduler | `rux-scheduler/docs/` — inventories and plans; `sch-*` is its own prefix |
