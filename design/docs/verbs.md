...
from the repository's root unless they say `design/`.
...
| | |
|---|---|
| How | Skill `design-page`. Copy the nearest `design/templates/*.html`; never start from scratch or from a guess. Markup is diffed against `data/carbon-*.json` (`node tools/diff-fragment.mjs <name>` in `design/`). |
| Serve | `npm run serve` → `http://localhost:8640/design/` — every app on one origin, the switcher filled from the hub's list. |
| Check | `npm run check -- --full` at the root, or `npm run verify` in `design/`. Then the browser gates, below. |
...
There is none of its own. `npm run verify` in `design/`, commit, and a push
...
under `design/tools/`. Serve the site, open the page to check, and in the
devtools console load the tool from the server so the file on disk is what
runs:

```js
(function(){ const r = new XMLHttpRequest();
  r.open('GET', '/design/tools/check-a11y.js?v=' + performance.now(), false); r.send();
