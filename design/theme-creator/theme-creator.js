...
function renderSavedThemes() {
  ...
  for (const t of saved) {
    ...
    row.querySelector('[data-act="delete"]').addEventListener('click', () => {
      if (!confirm(`Delete the saved theme "${t.id}"? This removes it from every Design app's account panel on this browser.`)) return;
...
function saveTheme() {
  ...
  status.textContent = `Saved "${state.name}" — ${Object.keys(tokens).length} changed tokens, on ${state.base}.${replaced ? ' Replaced your previous theme.' : ''} It is now an option in every Design app's account panel on this browser.`;
  renderSavedThemes();
  scheduleSave();
}
