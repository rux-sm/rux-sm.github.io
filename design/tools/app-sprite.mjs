...
// every inlined symbol is SOMEWHERE in what Design ships, never whether the
// paste is CURRENT and complete. `--check` is what asks that.
//
// WRITTEN 2026-09-11, FROM TWO COPIES THAT HAD ALREADY SPLIT. Scheduler
...
//
// IT IS NOT Notes's inline-sprite.mjs AND DOES NOT REPLACE IT. That
...
export function sprite({ root, ds, dirs = ['.'], check = false }) {
  ...
  if (check && stale) {
    console.log(`\n  ${stale} page(s) carry a sprite older than Design's. Run: node tools/sprite.mjs`);
    process.exit(1);
  }
  console.log(`\n  ${written} page(s) rewritten from Design's sprite (${(svg.match(/<symbol/g) ?? []).length} symbols)`);
}
