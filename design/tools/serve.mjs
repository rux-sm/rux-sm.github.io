...
// parent of this Design checkout. `/` is the hub -- the one folder named
...
// folder of the same name beside it: /design/ from design/, /scheduler/
// from scheduler/. A directory answers its index.html, as Pages does.
...
// /design/<path>, and this is the server on which that
...
// this repository. The old line read "at Design's root `/` is the sink, in an
// app it is index.html", written when Design had no landing page and the sink
// was the only thing to open. Design has had index.html for a while, and the
