...
- **No orphans.** Every document is linked from its folder's README.
...
- **Plans.** Work that spans more than one session gets a plan first, in the
  ...
  says go before building starts. Ticking a box is the only update, and the
  commit that ticks the last box deletes the plan.
...
- **Plan checks.** A plan lives only in a plans folder and has all four parts.
  A plan with every box ticked fails. A plan untouched for 30 days warns, read
...
- **Lowercase file names,** except `README.md`, `AGENTS.md`, `CLAUDE.md`,
  `LICENSE` and atlas content ids, which the naming plan covers.
...
- [ ] Build `tools/check-docs.mjs` and run it from `npm run check`: kinds,
      orphans, named files exist, lengths, status item length, plan shape and
      place, finished plans, stale plans as a local-only warning, and history
...
- [ ] Turn principle 10 into rules in `AGENTS.md`, each naming its check.
...
- [ ] Delete this plan in the commit that ticks the last box.
