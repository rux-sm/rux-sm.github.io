...
- **The editor picks it from the panel head's Trip actions menu,** and its
  hidden field is Carbon's dropdown, so its list reads like the bar menu's: a `scheduler-swatch` chip wearing the
