/* ==========================================================================
   ...
   NO DEPENDENCY ON Design's js/. Text areas, checkboxes and buttons are
...
(() => {
  ...
  // ---- controls -----------------------------------------------------------
  const answers = $('textarea[data-notes-answer]');
  const checks = $('input[data-notes-check]');
  const passes = $('input[data-notes-pass]');
  const reveals = $('button[data-notes-reveal]');
  const notes = root.querySelector('textarea[data-notes-notes]');
  const progress = root.querySelector('[data-notes-progress]');
  ...
  const revealFor = (ta) => ta.closest('.notes-q-field')?.querySelector('button[data-notes-reveal]') ?? null;

  const setRevealed = (btn, open) => {
    const panel = document.getElementById(btn.dataset.notesReveal);
    if (!panel) return;
    panel.hidden = !open;
  ...
  const sectionStatus = (section) => {
    const tas = $('textarea[data-notes-answer]', section);
    const boxes = [...$('input[data-notes-check]', section), ...$('input[data-notes-pass]', section)];
  ...
  const refresh = () => {
    const sections = $('[data-notes-section]');
    let done = 0;
    for (const section of sections) {
      const st = sectionStatus(section);
      if (st === 'done') done++;
      const tag = section.querySelector('[data-notes-status]');
      if (tag) {
  ...
  for (const btn of reveals) {
    btn.addEventListener('click', () => {
      ...
      state.r[btn.dataset.notesReveal] = open; save();
    });
  }
  if (notes) {
  ...
  const markdown = () => {
    ...
    const boxes = $('input[data-notes-check]');
    if (boxes.length) {
      const done = boxes.filter(b => b.checked).length;
      out.push(`## Checklist — ${done} of ${boxes.length} done`, '');
      for (const section of $('[data-notes-section]')) {
        const rows = $('.notes-q:has(input[data-notes-check])', section);
        if (!rows.length) continue;
        out.push(`**${text(section.querySelector('h2')).replace(/\s*(Not started|In progress|Done)$/, '')}**`);
        for (const item of rows) {
          const box = item.querySelector('input[data-notes-check]');
          out.push(`${box.checked ? '- [x] ' : '- [ ] '}${text(item.querySelector('.notes-q-label'))}`);
        }
        out.push('');
      }
    }
    for (const section of $('[data-notes-section]')) {
      ...
      for (const item of $('.notes-q', section)) {
        const label = text(item.querySelector('.notes-q-label'));
        const box = item.querySelector('input[data-notes-check]');
        out.push(`${box ? (box.checked ? '- [x] ' : '- [ ] ') : '- '}**${label}**`);
        for (const dt of $('.notes-meta-row', item)) {
          out.push(`  - ${text(dt.querySelector('dt'))}: ${text(dt.querySelector('dd'))}`);
        }
        const fields = $('textarea[data-notes-answer]', item);
      ...
      for (const pass of $('input[data-notes-pass]', section)) {
