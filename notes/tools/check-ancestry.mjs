...
// resolves fine and splits nothing. Answering it needs Design's 669 captured
// Carbon DOM stories -- 1.8 MB that are NOT vendored, deliberately, because a
// second copy of a rule Design owns is the drift this project is arranged
// against. `check-structure.mjs` refused that same trade for the same reason.
//
// SO THE GATE STAYS THERE AND IS POINTED HERE. Design's check-ancestry.mjs
...
//
// ROOTS ARE NAMED, NOT SWEPT. Design's `markupFiles()` reads .html directly in
...
//
// AGAINST THE Design IN THIS REPOSITORY. Until 2026-09-12 this unpacked
// Design's newest release tag with `git archive` and judged at that, because
...
//
//   node tools/check-ancestry.mjs          # every page, via ../design
...
if (!existsSync(GATE)) {
  console.error(`\n  unavailable: no Design checkout at ${DS}`);
...
// AGAINST THE Design IN THIS REPOSITORY, which is what these pages are served
...
console.log(`\n  check-ancestry against the Design in this repository`);
let status;
