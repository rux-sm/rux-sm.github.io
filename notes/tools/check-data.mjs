#!/usr/bin/env node
//
// Is data/atlas/ the bytes sync-export.sh delivered?
//
// WHY IT EXISTS. Every page here is generated from data/atlas/, and the only
// thing that sweeps that data for names, issue ids and evidence paths is
// atlas's emit.py, on the private side, when sync-export.sh runs it. A hand
...
// comparison: sync-export.sh records a hash of what it wrote, in the PIN
...
if (!recorded) {
  console.log('\n  data/atlas/PIN records no sha256, so nothing can be compared.');
  console.log('  Run sh tools/sync-export.sh; it writes one. Not a pass.\n');
  process.exit(1);
}
if (recorded !== hash) {
  console.log(`\n  data/atlas/ does not match the hash sync-export.sh recorded.`);
  ...
  console.log('  Re-run sh tools/sync-export.sh; never hand-edit data/atlas/.\n');
  process.exit(1);
}
console.log(`\n  ${files.length} file(s) under data/atlas/ match the hash sync-export.sh recorded (${hash.slice(0, 12)}…)`);
