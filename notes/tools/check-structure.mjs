...
// boxes and stack instead. Design hit this for real -- its tabs fragment nested
...
//
// WHY THIS IS DERIVED FROM CSS AND NOT FROM CAPTURES. Design answers structural
...
// and vendoring 1.45 MB of them to run a second copy of a rule Design already
...
//     refuses to make, so the whole class of defect Design's check-ancestry
...
const CSS = join(resolve(ROOT, process.env.DS ?? '../design'), 'css/rux.css');
...
// belong on one element. Run against Design's own six templates and its kitchen
...
function compoundPairs(css) {
  ...
  // the same reason Design intersects rather than unions.
