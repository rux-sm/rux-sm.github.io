#!/usr/bin/env node
//
// Splice Design's icon sprite into every page between its SPRITE markers,
// read from ../design, the design system's folder beside this one in the site
// repository (or DS=<dir>). Nothing is vendored.
//
// WHY A PAGE CANNOT JUST LINK THE SPRITE FILE. Referencing
// `/design/assets/icons.svg#i-name` from a <use> is broken in two
...
// page looks built and simply has no icons in it. Design records this as a
// check-icons fault and inlines into each of its templates for the same reason.
//
// WHY THIS IS NOT Design's tools/icons.mjs. That script regenerates the sprite
...
// already-built sprite, Design's own. So this only ever COPIES, and the
// sprite's content is Design's business entirely -- whatever is live.
//
...
//
// ONLY IF IT IS NOT ALREADY HIDDEN. Design's sprite ships with the style on it
