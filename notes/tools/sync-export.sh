#!/bin/sh
#
# Pull the walkthrough data from atlas, and record which commit produced it.
#
...
# session help and the screenshots of a licensed environment. A walkthrough that
...
# copied sat in data/atlas/ with nothing emitting it, and check-data.mjs
...
# would otherwise empty data/atlas/ and pass every gate after it. An empty
...
# one check and CI -- can refuse a data/atlas/ that did not come through
...
ls "$OUT"/*.json | wc -l | tr -d ' ' | xargs echo "  walkthroughs:"
