...
| `trip_updates` | the board, embedded in each trip for the bar's Updates mark and card; the trip panel's Updates tab, which adds one; the Save prompt | `trip_id` to `trips`, cascade; `created_at`, `actor_id` (default `auth.uid()`), `actor_name`, `body`, `kind` (update, nothing, imported), `changes` jsonb. What was said to the customer, newest first. Staff only, anon granted nothing, broadcast on realtime. |
