/* ==========================================================================
   ...
   --------------------------------------------------------------------------
   form.html?s=<token>&form=envelope&trip=<id>&leg=<leg> shows a driver,
   without a log-in, the envelope for their seat on one leg, as the forms
   page draws it. The card on their schedule link opens it; the card holds
   the basics and this holds the rest.
...
(() => {
  ...
  const form = forms?.FORMS.find(f => f.id === params.get('form') && f.id === 'envelope');
  ...
  // The envelope keeps its paper's width, as on the forms page, and `zoom` in
  // print.css takes it down whole to the room: nothing reflows, so what the
  // driver reads is laid out as it prints.
  ...
  async function load() {
    ...
    for (const card of cards) card.setAttribute('data-theme', 'g10');
  ...
  $('scheduler-sheet-title').textContent = 'Envelope';
  document.title = 'Envelope — Scheduler';
