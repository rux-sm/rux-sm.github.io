#!/usr/bin/env node
//
// Re-inlines Design's sprite into this app's pages. THE RULE IS NOT HERE:
// Design's tools/app-sprite.mjs holds it, read from the checkout beside this
...
// Scheduler's that had already drifted 13 lines from it. The only
...
if (!existsSync(join(ds, 'tools'))) {
  console.log(`  FAIL  sprite: no Design at ${ds} -- clone it beside this repository, or set DS=<dir>`);
  process.exit(1);
}
// RUX-DS IS THERE BUT PREDATES THE RULE. CI checks Design out at its NEWEST
...
if (!existsSync(join(ds, 'tools/app-sprite.mjs'))) {
  console.log(`  FAIL  sprite: Design at ${ds} has no tools/app-sprite.mjs.`);
  console.log('        It is on main and not yet in a release. CI reads the newest tag,');
  console.log('        so this passes locally and fails there until Design is tagged.');
  process.exit(1);
}
const { sprite } = await import(new URL('tools/app-sprite.mjs', `file://${ds}/`).href);
sprite({ root, ds, dirs: ['.', 'account', 'scheduler'], check: process.argv.includes('--check') });
